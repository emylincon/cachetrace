import hashlib
import math
# This is an implementation of FBR
...
class CountChain:      # replace and maintain count and maintain boundary
    ...
    def __init__(self, cache_size):
        ...
        self.total_count = 0
        self.a_max = 11  # maximum average of counts
        self.c_max = 11  # maximum chain count
    ...
    def maintain_count(self):
        if self.average_count > self.a_max:
            ...
            new_chain = {i: LRUChain() for i in range(1, max(self.chain)+1)}
    ...
    def find_match(self, count, old, new, boundary, new_node):
        tails = [boundary.prev] + [self.chain[i].tail for i in range(count + 1, max(self.chain) + 1)] + [self.chain[i].tail for i in range(1, count)] + [self.chain[count].tail]
        # print(tails)
    ...
    def maintain_boundary(self, new_node, status, replace):    # status => hit or miss
        if self.length > sum(self.section_count):
            ...
            if (status == 'miss' and self.length == self.cache_size and replace is True) or (status == 'hit' and new_node.old is True):
                ...
                if self.new_boundary is None:
                    print(f'{nt.details} <> dis-> ', self.details_display())
        ...
        def find_match(n_node):
            while n_node is not None:
                ...
                if n_node.old is True:
                    return n_node
                n_node = n_node.prev
        for no in range(1, max(self.chain)+1):
            chain = self.chain[no]
    ...
    def push(self, data):
        ...
        if new_node.count not in self.chain:
